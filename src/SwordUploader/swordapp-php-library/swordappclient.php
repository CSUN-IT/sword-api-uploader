...
class SWORDAPPClient {
	...
	function servicedocument($sac_url, $sac_u, $sac_p, $sac_obo) {
		...
		array_push($headers, "User-Agent: SWORDAPP PHP library (version " . self::API_VERSION . ") " .
	...
	function deposit($sac_url, $sac_u, $sac_p, $sac_obo, $sac_fname,
		...
		if (($sac_status >= 200) && ($sac_status < 300)) {
			try {
				...
				if (!$this->checkEmptyNamespaces($sac_ns)) {
					$sac_ns['atom'] = null;
					$sac_ns['sword'] = null;
				}
	...
	/**
	 * Checks to see if we have no namespace on our file
	 */
	private function checkEmptyNamespaces($namespaces) {
		if (isset($namespaces['atom']) && isset($namespaces['sword'])) {
			return true;
		}
		return false;
	}
